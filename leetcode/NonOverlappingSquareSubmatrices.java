...
class Solution {
    ...
    // for a given range
    boolean exists(int B[][], int k, int top, int left, int bottom, int right, HashMap<String, Boolean> DP) {

        if(top > bottom || left > right) {
            return false; // not possible, invalid region
        }

        String key = top + "-" + left + "-" + bottom + "-" + right;
        ...
        for(int i = top; i <= bottom - k + 1; i ++) {
            for(int j = left; j <= right - k + 1; j ++) {
                ...
                if(a == k * k) {
                    ...
                    DP.put(key, true); // store it for further use
                    return true;
                }
            }
        }

        DP.put(key, false); // store it for further use
        return false;
    }
    ...
    boolean possible(int B[][], int k) {
        ...
        int N = B.length - 1;
        int M = B[0].length - 1;
        HashMap<String, Boolean> DP = new HashMap<>(); // DP to store repetitive regions results
        for(int i = 0; i <= N - k; i++) {
            for(int j = 0; j <= M - k; j++) {
                ...
                int a = findArea(B, x1, y1, x2, y2);
                if(a == k * k) {
                    // then find another non-overlaping square in two overlapping partitions
                    // as we keep on moving from top to bottom so we will check right and bottom side
                    int regions[][] = new int[][]{
                        {0, y2 + 1, N - 1, M - 1},
                        {x2 + 1, 0, N - 1, M - 1}
                    };
                    for(int region[] : regions) {
                        boolean found = exists(B, k, region[0], region[1], region[2], region[3], DP);
