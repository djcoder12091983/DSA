...
// Wrong -- compare based on turns if same turns then sort based on dist
// TODO -- need to analyze correctness

// TODO -- may need to think of solve using Backtracking!
...
class Solution {
    ...
    // compare based on turns if same turns then sort based on dist
    // TODO -- need to analyze correctness
    int compare(Node x, Node y) {
        int c = x.turns - y.turns;

        if(c != 0) {
            return c;
        }

        // sort based on dist if shared same turns
    ...
    public int minCost(int[][] G, int k) {
        ...
        // compare based on turns if same turns then sort based on dist
        // TODO -- need to analyze correctness
        PriorityQueue<Node> search = new PriorityQueue<>((x, y) -> compare(x, y));
